#!/usr/bin/env node
// CI byte-identical reproducibility check for cpp.wasm.
...
const committed = await readFile(path.join(repoRoot, "cpp.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed cpp.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-cpp-verify-"));
...
console.log(`rebuilt cpp.wasm sha256: ${rebuiltHash}`);
