#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-cpp.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-cpp-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-cpp.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as cpp.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "cpp.wasm"));
const bytes = (await readFile(path.join(repoRoot, "cpp.wasm"))).length;
console.log(`cpp.wasm: ${bytes} bytes (built from ${pin})`);
